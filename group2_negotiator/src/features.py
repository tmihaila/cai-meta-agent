...
def domain_features(issues) -> dict:
    ...
    return {
        ...
        "log_outcome_space_size": sum(math.log2(c) for c in cardinalities),  # log2 of total combinations
    }


def profile_features(ufun: LUFun) -> dict:
    """Extract features from the agent's utility function (issue weights and value spreads)."""
    ...
    return {
        ...
        "weight_entropy": float(entropy / max_entropy),  # 1.0 = uniform, 0.0 = single dominant issue
        "avg_value_std": float(np.mean(stds)),
        "weight_concentration": float((w_norm ** 2).sum()),  # Herfindahl index: 1/n = uniform, 1.0 = one issue dominates
    }
...
# Ordered column names matching the NN input; must stay in sync with extract_all_features
